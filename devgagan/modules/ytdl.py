...
import aiohttp  # For async thumbnail downloading
import logging
...
# Non-blocking download thumbnail function
...
# Non-blocking wrapper for yt_dlp download
...
async def process_audio(client, event, url, cookies_env_var=None):
    ...
    try:
        ...
        # await progress_message.edit("**__Starting Upload...__**")
...
async def fetch_video_info(url, ydl_opts, progress_message, check_duration_and_size):
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if check_duration_and_size:
            ...
            if duration and duration > 3 * 3600:  # 3 hours
                await progress_message.edit("**❌ __Video is longer than 3 hours. Download aborted...__**")
                return None

            # Check estimated size (max 2GB)
            estimated_size = info_dict.get('filesize_approx', 0)
            if estimated_size and estimated_size > 2 * 1024 * 1024 * 1024:  # 2GB
...
async def handler(event):
    ...
    # Check if user already has an ongoing download
    if user_id in ongoing_downloads:
    ...
    # Check URL and set cookies based on site
    try:
        if "instagram.com" in url:
        ...
        # Remove user from ongoing downloads
        ongoing_downloads.pop(user_id, None)
...
async def process_video(client, event, url, cookies_env_var, check_duration_and_size=False):
    ...
    # Write cookies to a temporary file, if cookies exist
    temp_cookie_path = None
    if cookies:
        with tempfile.NamedTemporaryFile(delete=False, mode='w', suffix='.txt') as temp_cookie_file:
            temp_cookie_file.write(cookies)
    ...
    try:
        ...
        # await progress_message.edit("**__Uploading video...__**")
